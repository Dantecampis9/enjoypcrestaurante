...
// vuelve a aparecer. Los datos capturados (si el usuario acepta) se guardan SOLO
// en localStorage bajo `enjoy-lead` — no hay backend ni servicio externo conectado.
// Si en el futuro se conecta un servicio real (Formspree, etc.), reemplazar el
// localStorage.setItem(STORAGE_LEAD, ...) del listener de submit por el envío real.
(function () {
  const STORAGE_SEEN = "enjoy-lead-seen";
  const STORAGE_LEAD = "enjoy-lead";
  ...
  function buildModal() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      localStorage.setItem(STORAGE_LEAD, JSON.stringify({ name, email, date: new Date().toISOString() }));
      close();
    });
